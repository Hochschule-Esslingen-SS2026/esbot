...
async function loadMessages(sessionId) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/Sessions/messages/${sessionId}`);
        if (response.ok) {
...
function renderMessage(msg) {
    ...
    div.setAttribute('data-testid', isUser ? 'user-message' : 'assistant-message');
...
async function sendMessage() {
    ...
    messageInput.value = '';
    renderMessage({ role: true, content: text, id: 'temp', timestamp: new Date().toISOString() });
    scrollToBottom();
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/Sessions/${currentSessionId}/messages`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userSessionId: currentSessionId, question: text })
        });
...
async function generateQuiz() {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/Sessions/${currentSessionId}/quiz`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userSessionId: currentSessionId, topic: topic })
        });
...
async function submitAnswer() {
    if (selectedQuizOption === null || !currentSessionId || !currentQuestionId) return;
    
    submitAnswerBtn.disabled = true;
    submitAnswerBtn.textContent = 'Submitting...';
    
    try {
        const response = await fetch(`${API_BASE_URL}/Sessions/${currentSessionId}/quiz/${currentQuestionId}/answer`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ /* AnswerEvaluationRequest schema is empty in swagger, sending empty object or selected index */ answerIndex: selectedQuizOption })
        });
